package com.expenses.manager.presentation.statistics.daily
...
class DailyStatisticsViewModel @Inject constructor(
    ...
    private fun fetchLineChartStatistics(year: Int, month: Int, day: Int) {
        viewModelScope.launch(Dispatchers.IO) {
            lineChartStatisticsLiveData.postValue(
                expensesStatisticsService.getLineChartStatisticsOfDay(
                    year,
                    month,
                    day,
                    categoriesFilters
    ...
    private fun fetchLegendData(year: Int, month: Int, day: Int) {
        viewModelScope.launch(Dispatchers.IO) {
            legendLiveData.postValue(
                expensesStatisticsService.getNonZeroCategoryOfDay(
                    year,
                    month,
                    day
                ).apply {
    ...
    private fun fetchExpenses(year: Int, month: Int, day: Int) {
        viewModelScope.launch(Dispatchers.IO) {
            expensesLiveData.postValue(
                expensesDao.getExpensesByDayWithFilter(
                    year,
                    month,
                    day,
                    categoriesFilters
